...
public:
    unsigned int count() const; // return number of nodes
...
protected:
    ...
    bool insertAt(unsigned int index, Q3PtrCollection::Item); // add item at i'th position
    ...
    bool removeAt(unsigned int); // remove item at i'th position
    bool replaceAt(unsigned int, Q3PtrCollection::Item); // replace item at position i with item
    Q3PtrCollection::Item takeNode(Q3LNode*); // take out node
    Q3PtrCollection::Item take(); // take out current item
    Q3PtrCollection::Item takeAt(unsigned int index); // take out item at i'th pos
    ...
    unsigned int containsRef(Q3PtrCollection::Item) const; // get number of exact matches
    unsigned int contains(Q3PtrCollection::Item) const; // get number of equal matches

    Q3PtrCollection::Item at(unsigned int index); // access item at i'th pos
...
private:
    ...
    Q3LNode* locate(unsigned int); // get node at i'th pos
    Q3LNode* unlink(); // unlink node
};

inline unsigned int Q3GList::count() const
{
    return numNodes;
}
...
inline Q3PtrCollection::Item Q3GList::at(unsigned int index)
{
...
protected:
    ...
    Q3PtrCollection::Item operator+=(unsigned int); // move n positions forward
    Q3PtrCollection::Item operator--(); // move to prev item (prefix)
    Q3PtrCollection::Item operator-=(unsigned int); // move n positions backward
...
public:
    inline Q3GListStdIterator(Q3LNode* n)
        : node(n)
    {
    }
