...
//public:
//    QAbstractScrollAreaWidget(Q3ScrollView* parent=0, const char* name=0, Qt::WindowFlags f = Qt::WindowFlags())
//        : QWidget(parent)
//    {
...
public:
    Q3ScrollViewData(Q3ScrollView* parent, int vpwflags) :
        ...
        viewport(new QWidget(parent, QFlag(vpwflags))),
        clipped_viewport(0),
        flags(vpwflags),
...
//        d->hbar->setSteps(Q3ScrollView::d->hbar->lineStep(), portw);
    ...
    int bottom;
    bool reverse = QApplication::layoutDirection();
...
{
    QWheelEvent ce(viewport()->mapFromGlobal(e->globalPosition()),
                    e->globalPosition(),
                   e->pixelDelta(),
                   e->angleDelta(),
                   e->buttons(),
...
//        if (e->orientation() == Horizontal && horizontalScrollBar())
        if (e->angleDelta().y() == 0 && horizontalScrollBar())
            horizontalScrollBar()->event(e);
//        else  if (e->orientation() == Vertical && verticalScrollBar())
        else  if (e->angleDelta().x() == 0 && verticalScrollBar())
...
{
//    e->setPoint(viewportToContents(e->position().toPoint()));
//    contentsDropEvent(e);
//    e->setPoint(contentsToViewport(e->position().toPoint()));
}
...
//        e->globalPos(), e->delta(), e->state());
    ...
    QWheelEvent ce(viewportToContents(e->position().toPoint()),
                    e->globalPosition(),
                   e->pixelDelta(),
                   e->angleDelta(),
                   e->buttons(),
...
void Q3ScrollView::enableClipper(bool y)
{
    ...
    if (y) {
        d->clipped_viewport = new QClipperWidget(clipper(), QFlag(d->flags));
...
//        d->clipped_viewport->setBackgroundMode(d->viewport->backgroundMode());
        ...
        d->viewport->setAttribute(Qt::WA_NoSystemBackground, true);
