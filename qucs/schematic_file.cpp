...
int Schematic::saveSymbolCpp (void)
{
  ...
  QString cppfile = info.dir().path() + QDir::separator() + DataSet;
  QFile file (cppfile);
...
{
  QFileInfo info (DocName);
  QString jsonfile = info.dir().path() + QDir::separator()
...
bool Schematic::loadDocument()
{
  ...
  if(!misc::checkVersion(Line)) { // wrong version number ?
    ...
    QString msg = "Wrong document version \n";
    msg += DocName;
    msg += "\nTry to open it anyway?";
    result = QMessageBox::warning(0,
                                  QObject::tr("Warning"),
                                  QObject::tr( msg.toLatin1().data()),
  ...
  while((pc = it.current()) != 0) {
    ...
    if(pc->Model == "Sub")
    {
      ...
      if (r)
      {
        ...
        FileList.insert(f, sub); // CRITICAL_PRABHU
      }
      delete d;
      if(!r)
      {
                                      ...
                                      int NumPorts)
{
  ...
  if(creatingLib) {
    ...
    ofile.setFileName(f);
    if(!ofile.open(QIODeviceBase::ReadOnly)) {
  ...
  for(it_name = SubcircuitPortNames.begin(),
    ...
    if(*it_name == " ") {
      SubcircuitPortNames.removeOne(*it_name);// CRITICAL_PRABHU
      SubcircuitPortTypes.removeOne(*it_type);
    } else {
