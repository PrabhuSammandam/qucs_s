   ...
   : QDialog( parent )
{
   ...
   if( parent )
   {
      ...
      defaultDataSet = Info.dir().path() + QDir::separator() + s->DataSet;
   }
   else
   {
   ...
   if( Diag->Name.at( 0 ) != 'T' ) // not tabular or timing diagram
   {
      ...
      // gp->addMultiCellWidget(new QLabel(NameY+" "+tr("Label:"), Tab2), Row,Row,0,0);
      ...
      if( ( Diag->Name != "Smith" ) && ( Diag->Name != "Polar" ) )
      {
         ...
         // gp->addMultiCellWidget(new QLabel(NameZ +" "+tr("Label:"), Tab2), Row,Row,0,0);
      ...
      gp->addWidget( new QLabel( tr( "<b>Label text</b>: Use LaTeX style for special characters, e.g. \\tau" ), Tab2 ), Row, 0, 0, 1 );
      // gp->addMultiCellWidget(new QLabel(tr("<b>Label text</b>: Use LaTeX style for special characters, e.g. \\tau"),Tab2),  Row,Row,0,2);
      Row++;

      if( Diag->Name != "Rect3D" )
      {
         GridOn = new QCheckBox( tr( "show Grid" ), Tab2 );
         gp->addWidget( GridOn, Row, 0, 0, 2 );
         ...
         gp->addWidget( GridColorButt, Row, 1, 0, 2 );
         // gp->addMultiCellWidget(GridColorButt, Row,Row,1,2);
         Row++;

         auto pp = GridColorButt->palette();
         pp.setColor( GridColorButt->backgroundRole(), Qt::blue );
         GridColorButt->setPalette( pp );
         ...
         gp->addWidget( GridStyleBox, Row, 1, 0, 1 );
      ...
      if( ( Diag->Name.left( 4 ) == "Rect" ) || ( Diag->Name == "Curve" ) )
      {
         ...
         if( Diag->Name == "Rect3D" )
         {
            ...
            gp->addWidget( hideInvisible, Row, 0, 0, 2 );
            // gp->addMultiCellWidget(hideInvisible, Row,Row,0,2);
            Row++;

            QLabel *LabelRotX = new QLabel( tr( "Rotation around x-Axis:" ), Tab2 );
            auto    pp        = LabelRotX->palette();
            pp.setColor( LabelRotX->backgroundRole(), Qt::red );
            LabelRotX->setPalette( pp );
            ...
            pp = LabelRotY->palette();
            pp.setColor( LabelRotY->backgroundRole(), Qt::green );
            LabelRotY->setPalette( pp );
            ...
            pp = LabelRotZ->palette();
            pp.setColor( LabelRotZ->backgroundRole(), Qt::blue );
            LabelRotZ->setPalette( pp );
   ...
   if( ColorButt )
   {
      if( !currentGraph )
      {
         ...
         auto    pp = ColorButt->palette();
         pp.setColor( ColorButt->backgroundRole(), selectedColor );
         ColorButt->setPalette( pp );
...
void DiagramDialog::slotReadVarsAndSetSimulator( int )
{
   ...
   Info.setFile( Info.dir().path() + QDir::separator() + DocName );

   if( Info.exists() )
   {
      ChooseSimulator->addItem( "Qucsator (built-in)" );
   }

   Info.setFile( Info.dir().path() + QDir::separator() + DocName + ".ngspice" );
   ...
   Info.setFile( Info.dir().path() + QDir::separator() + DocName + ".xyce" );
   ...
   Info.setFile( Info.dir().path() + QDir::separator() + DocName + ".spopus" );
...
void DiagramDialog::slotReadVars( int )
{
   ...
   QFile file( Info.dir().path() + QDir::separator() + DocName );
...
void DiagramDialog::slotTakeVar( QTableWidgetItem *Item )
{
   ...
   if( Diag->Name != "Tab" )
   {
      if( Diag->Name != "Truth" )
      {
         g->Color = ColorButt->palette().color( ColorButt->backgroundRole() );// ColorButt->paletteBackgroundColor();
...
void DiagramDialog::SelectGraph( Graph *g )
{
   ...
   if( Diag->Name != "Tab" )
   {
      if( Diag->Name != "Truth" )
      {
         ...
         ColorButt->setStyleSheet( stylesheet );
         auto    pp = ColorButt->palette();
         pp.setColor( ColorButt->backgroundRole(), g->Color );
         ColorButt->setPalette( pp );
...
void DiagramDialog::slotApply()
{
   if( Diag->Name.at( 0 ) != 'T' ) // not tabular or timing
   {
      ...
      if( GridColorButt )
      {
         auto cc = GridColorButt->palette().color( GridColorButt->backgroundRole() );

         if( Diag->GridPen.color() != cc )
         {
            Diag->GridPen.setColor( cc );
            changed = true;
         }
      }
...
void DiagramDialog::slotSetColor()
{
   ...
   pp.setColor( ColorButt->backgroundRole(), c );
   ColorButt->setPalette( pp );
...
{
   QColor c = QColorDialog::getColor( GridColorButt->palette().color( GridColorButt->backgroundRole() ), this );
   ...
   pp.setColor( GridColorButt->backgroundRole(), c );
   GridColorButt->setPalette( pp );
